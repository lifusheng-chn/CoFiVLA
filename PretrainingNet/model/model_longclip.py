...
class CLIP(nn.Module):
    def __init__(self,
                 ...
                 load_from_clip: bool # add
                 ):
        super().__init__()

        self.context_length = 77

        if isinstance(vision_layers, (tuple, list)): #  是否是一个元组（tuple）或列表（list）类型的对象。
    ...
    def forward(self, image, text_long,text_short,rank):
        ...
        # 使用 self.PCA 将 image_features_long 特征向量的维度减少到 32 维。这是通过主成分分析（PCA）完成的，用于降维。
        # image_features_short = self.PCA(image_features_long, 32)
        image_features_short = image_features_long

            
        # image_feat_all_long = torch.cat(torch.distributed.all_gather(image_features_long), dim=0)#gather with grad
        # image_features_all_short = torch.cat(torch.distributed.all_gather(image_features_short), dim=0)
        # text_feat_all_long = torch.cat(torch.distributed.all_gather(text_features_long), dim=0)
        # text_feat_all_short = torch.cat(torch.distributed.all_gather(text_features_short), dim=0)
        
        # # 创建一个空的张量来接收结果(多卡分布式训练)
        # image_feat_all_long = [torch.empty_like(image_features_long) for _ in range(torch.distributed.get_world_size())]
        # torch.distributed.all_gather(image_feat_all_long, image_features_long)

        # image_feat_all_long = torch.cat(image_feat_all_long, dim=0)

        # image_features_all_short = [torch.empty_like(image_features_short) for _ in range(torch.distributed.get_world_size())]
        # torch.distributed.all_gather(image_features_all_short, image_features_short)

        # image_features_all_short = torch.cat(image_features_all_short, dim=0)

        # text_feat_all_long = [torch.empty_like(text_features_long) for _ in range(torch.distributed.get_world_size())]
        # torch.distributed.all_gather(text_feat_all_long, text_features_long)

        # text_feat_all_long = torch.cat(text_feat_all_long, dim=0)

        # text_feat_all_short = [torch.empty_like(text_features_short) for _ in range(torch.distributed.get_world_size())]
        # torch.distributed.all_gather(text_feat_all_short, text_features_short)

        # text_feat_all_short = torch.cat(text_feat_all_short, dim=0)
        # #------------------------------------------------------------------------------
