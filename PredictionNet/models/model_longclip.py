...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        for i in range(self.transformer.layers):
            ...
            if (i + 1) % 3 == 0:  # 每四层获取一次输出
                # 对每个输出执行 permute
                outputs.append(x)

        permuted_outputs = [out.permute(1, 0, 2) for out in outputs]
        outputs = [self.ln_post(out) for out in permuted_outputs]
        # image_embed = x[:, 1: , :]  # 全部特征
        # image_embed = self.ln_post(x)
        # image_embed = self.ln_post(image_embed)
...
class CLIP(nn.Module):
    ...
    def forward(self, image, text_long,text_short):
        image_features_long, image_all_features_long = self.encode_image(image)
        ...
        bs = image.size(0)
        targets = torch.arange(len(image),dtype=torch.long,device=image.device)
